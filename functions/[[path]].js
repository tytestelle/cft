// Cloudflare Pages Functions - 增强安全文本存储系统 V3（酷9播放器专用版）
...
<body>
    ...
    <div class="ku9-feature">
        ...
        <p>2. 支持User-Agent包含"okhttp/1.31"的客户端</p>
        <p>3. 支持带酷9专用令牌的访问</p>
        <p>4. 其他客户端需使用通用访问令牌</p>
    </div>
    
    <div class="token-generator">
        <h4>🔑 令牌系统说明：</h4>
        <p><strong>酷9播放器专用令牌：</strong> <code id="ku9Token">ku9_secure_token_2024</code></p>
        <p><strong>通用访问令牌：</strong> <code id="accessToken">default_access_token_2024</code></p>
        <p><strong>管理令牌：</strong> <code id="manageToken">default_manage_token_2024</code></p>
        <p>使用方式：<code>/z/文件名?token=令牌值</code></p>
    </div>
    ...
    <form id="uploadForm">
        ...
        <button type="button" onclick="generateTokenLinks()">生成令牌链接</button>
    </form>
    ...
    <div id="linkDisplay" style="display:none;">
        ...
        <div id="tokenLinks" style="display:none; margin-top:15px;">
            <div class="success-message">🔗 带令牌的链接：</div>
            <p><strong>酷9专用：</strong> <a id="ku9Link" href="" target="_blank"></a> <button class="copy-btn" onclick="copyLink('ku9Link')">复制</button></p>
            <p><strong>通用令牌：</strong> <a id="accessLink" href="" target="_blank"></a> <button class="copy-btn" onclick="copyLink('accessLink')">复制</button></p>
        </div>
        
        <div class="encryption-info">
            <strong>🔒 安全说明：</strong><br>
            1. 酷9播放器可直接访问原始链接<br>
            2. 其他播放器/软件需使用通用令牌链接<br>
            3. 抓包软件自动屏蔽<br>
            4. 支持okhttp/1.31客户端
        </div>
    </div>
...
<ul>
    ...
    <script>
        ...
        function generateTokenLinks() {
            ...
            const baseUrl = link.split('/z/')[0] + '/z/';
            const filename = link.split('/z/')[1];
            
            // 酷9专用令牌
            const ku9Token = 'ku9_secure_token_2024';
            const ku9Link = baseUrl + filename + '?token=' + ku9Token;
            document.getElementById('ku9Link').href = ku9Link;
            document.getElementById('ku9Link').textContent = ku9Link;
            
            // 通用访问令牌
            const accessToken = 'default_access_token_2024';
            const accessLink = baseUrl + filename + '?token=' + accessToken;
            document.getElementById('accessLink').href = accessLink;
            document.getElementById('accessLink').textContent = accessLink;
            
            document.getElementById('tokenLinks').style.display = 'block';
        }
        ...
        function copyLink(linkId) {
            let link;
            if (linkId) {
                link = document.getElementById(linkId).href;
            } else {
                link = document.getElementById('linkAnchor').href;
            }
...
<div class="login-container">
  ...
  <div class="security-note">
    <h4>令牌系统说明：</h4>
    <p><strong>管理令牌：</strong> <code>default_manage_token_2024</code></p>
    <p><strong>酷9专用令牌：</strong> <code>ku9_secure_token_2024</code></p>
    <p><strong>通用访问令牌：</strong> <code>default_access_token_2024</code></p>
    <p>酷9播放器和带酷9令牌的客户端可直接访问真实内容</p>
  </div>
</div>
...
async function getSearchHTML(request, env, managementToken) {
  ...
  if (searchResults.length > 0) {
    ...
    for (const r of searchResults) {
      ...
      // 酷9专用令牌链接
      const ku9TokenLink = '/z/' + encodeURIComponent(r.name) + '?token=ku9_secure_token_2024';
      // 通用令牌链接
      const accessTokenLink = '/z/' + encodeURIComponent(r.name) + '?token=default_access_token_2024';
      
      fileListHTML += `
<div class='file-item'>
  <input type='checkbox' name='selected_files[]' value='${r.name.replace(/"/g, '&quot;')}'>
  <a href='${ku9TokenLink}' class='file-link' target='_blank'>${r.name}</a>
  ...
  <button type='button' class='token-btn' onclick='showTokens("${r.name.replace(/"/g, '&quot;')}", "${ku9TokenLink}", "${accessTokenLink}")'>🔗令牌</button>
...
.token-btn{background:none;border:none;color:#5cb85c;cursor:pointer;font-size:14px;padding:0 4px;}
.token-btn:hover{color:#449d44;}
...
<div class="ku9-feature">
  ...
  <ul class="security-list">
    <li>✅ 酷9播放器可直接访问真实内容</li>
    <li>✅ 支持User-Agent包含"okhttp/1.31"的客户端</li>
    <li>✅ 支持带酷9专用令牌的访问</li>
    <li>✅ 其他软件需使用通用访问令牌</li>
    <li>✅ 自动屏蔽抓包软件</li>
  </ul>
...
<div class="token-info">
  <h4>🔑 令牌系统说明：</h4>
  <p><strong>酷9播放器专用令牌：</strong> <code>ku9_secure_token_2024</code></p>
  <p><strong>通用访问令牌：</strong> <code>default_access_token_2024</code></p>
  <p><strong>管理令牌：</strong> <code>${managementToken}</code></p>
  <p style="font-size: 12px; color: #666;">使用方式：<code>/z/文件名?token=令牌值</code></p>
</div>
...
// 显示令牌功能
function showTokens(filename, ku9Link, accessLink) {
    const existingModal = document.getElementById('tokenModal');
    const existingOverlay = document.getElementById('tokenOverlay');
    ...
    modal.id = 'tokenModal';
    modal.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:600px;max-width:90%;padding:15px;background:white;border:1px solid #ccc;box-shadow:0 0 12px rgba(0,0,0,0.3);z-index:1000;';
    
    const domain = window.location.origin;
    const ku9FullLink = domain + ku9Link;
    const accessFullLink = domain + accessLink;
    
    modal.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span><strong>访问令牌：</strong>' + filename + '</span><span class="close-btn" style="cursor:pointer;color:#d9534f;font-weight:bold;font-size:16px;">×</span></div><div style="margin-bottom:15px;"><p><strong>酷9播放器专用令牌：</strong></p><div style="padding:8px;background:#f9f9f9;border:1px solid #ddd;border-radius:4px;margin-bottom:8px;font-family:monospace;font-size:12px;word-break:break-all;">' + ku9FullLink + '</div><button type="button" class="search-btn" onclick="copyToClipboard(\\'' + ku9FullLink + '\\')">📋 复制链接</button></div><div><p><strong>通用访问令牌：</strong></p><div style="padding:8px;background:#f9f9f9;border:1px solid #ddd;border-radius:4px;margin-bottom:8px;font-family:monospace;font-size:12px;word-break:break-all;">' + accessFullLink + '</div><button type="button" class="search-btn" onclick="copyToClipboard(\\'' + accessFullLink + '\\')">📋 复制链接</button></div>';
...
async function handleSecureFileDownload(filename, request, env) {
  try {
    ...
    // 定义令牌常量
    const KU9_SECURE_TOKEN = 'ku9_secure_token_2024';
    const DEFAULT_ACCESS_TOKEN = 'default_access_token_2024';
    const DEFAULT_MANAGE_TOKEN = 'default_manage_token_2024';

    // 1. 检查管理令牌（最高权限）
    if (managementToken && managementToken === DEFAULT_MANAGE_TOKEN) {
      return sendOriginalContent(safeFilename, content, 'management');
    }

    // 2. 检查酷9专用令牌
    if (token && token === KU9_SECURE_TOKEN) {
      return sendOriginalContent(safeFilename, content, 'ku9-token');
    }

    // 3. 检查通用访问令牌
    if (token && token === DEFAULT_ACCESS_TOKEN) {
      return sendOriginalContent(safeFilename, content, 'access-token');
    }
    ...
    if (isSniffingTool) {
      // 抓包软件，返回加密内容
      return sendEncryptedContent(safeFilename, content, 'sniffing-tool');
    }

    // 5. 酷9播放器检测（宽松检测）
    const ku9Keywords = [
      'ku9player', 'k9player', 'ku9 player', 'k9 player',
      'com.ku9.player', 'com.k9.player', 'okhttp/1.31'
    ];
    ...
    // 额外检测：包含okhttp的Android客户端（可能也是酷9）
    if (!isKu9Player && lowerUA.includes('okhttp') && 
    ...
    // 如果有明确的酷9标识，返回原始内容
    if (isKu9Player) {
      return sendOriginalContent(safeFilename, content, 'ku9-player');
    }

    // 6. 其他客户端（包括其他播放器、浏览器等）返回加密内容
    return sendEncryptedContent(safeFilename, content, 'other-client');
...
function sendOriginalContent(filename, content, clientType) {
  ...
  return new Response(content, {
    headers: {
      ...
      'Pragma': 'no-cache',
      'Expires': '0',
      'X-Client-Type': clientType || 'unknown'
    }
  });
}

// 发送加密内容
function sendEncryptedContent(filename, content, clientType = 'unknown') {
  let contentType = 'text/plain; charset=utf-8';
  let finalContent = '';
  
  if (clientType === 'sniffing-tool') {
    // 针对抓包软件的特别处理
...
# 1. 酷9播放器（直接访问或使用酷9专用令牌）
# 2. 通用访问令牌：default_access_token_2024
# 3. 酷9专用令牌：ku9_secure_token_2024
...
# 访问示例：
# ${filename}?token=ku9_secure_token_2024
# ${filename}?token=default_access_token_2024`;
  } else if (filename.endsWith('.m3u') || filename.endsWith('.m3u8')) {
    contentType = 'audio/x-mpegurl; charset=utf-8';
    finalContent = `#EXTM3U
#EXT-X-VERSION:3
...
# 支持的访问方式：
# 1. 酷9播放器（自动识别）
# 2. 酷9专用令牌：ku9_secure_token_2024
# 3. 通用访问令牌：default_access_token_2024

# 访问示例：
# 原始链接 + ?token=ku9_secure_token_2024

# 错误代码: ACCESS_TOKEN_REQUIRED
...
#EXT-X-ENDLIST`;
    ...
    finalContent = JSON.stringify({
      ...
      supported_tokens: {
        ku9_secure_token: "ku9_secure_token_2024",
        default_access_token: "default_access_token_2024"
      },
      access_url_examples: [
        filename + "?token=ku9_secure_token_2024",
        filename + "?token=default_access_token_2024"
      ],
      note: "酷9播放器可自动识别，无需令牌"
    }, null, 2);
  } else {
    finalContent = `🚫 安全保护已启用
...
注意：酷9播放器使用okhttp/1.31客户端可直接访问，无需令牌。`;
  }
  ...
  return new Response(finalContent, {
    headers: {
      ...
      'X-Content-Type-Options': 'nosniff',
      'X-Security': 'Enabled',
      'X-Access-Token-Required': 'true',
      'X-Supported-Tokens': 'ku9_secure_token_2024, default_access_token_2024',
      'Cache-Control': 'no-cache, no-store, must-revalidate'
    }
  });
}
  ...
  try {
    ...
    try {
      ...
      const metadata = {
        ...
        security: {
          enabled: true,
          allowed_tokens: ['ku9_secure_token_2024', 'default_access_token_2024'],
          allowed_clients: ['ku9_player', 'okhttp/1.31']
        }
      };
      await env.MY_TEXT_STORAGE.put('meta_' + safeFilename, JSON.stringify(metadata));
      ...
      const ku9TokenLink = link + '?token=ku9_secure_token_2024';
      const accessTokenLink = link + '?token=default_access_token_2024';

      return new Response(JSON.stringify({
        success: true,
        fileLink: link,
        ku9TokenLink: ku9TokenLink,
        accessTokenLink: accessTokenLink,
